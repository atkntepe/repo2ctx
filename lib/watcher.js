...
export class FileWatcher {
  constructor(options = {}) {
    ...
    this.pendingProcessPromises = new Map();
    ...
    const timer = setTimeout(() => {
      const promise = this.processChanges(watchPath, eventType, filePath)
        ...
        .finally(() => {
          ...
          this.pendingProcessPromises.delete(watchPath);
        });

      this.pendingProcessPromises.set(watchPath, promise);
    }, this.config.debounceDelay);
    
    this.debounceTimers.set(watchPath, timer);
  }

  async flushPendingChanges(watchPath = null) {
    if (watchPath) {
      const pendingProcessPromise = this.pendingProcessPromises.get(watchPath);
      if (pendingProcessPromise) {
        await pendingProcessPromise;
      }
      return;
    }

    if (this.pendingProcessPromises.size > 0) {
      await Promise.all(this.pendingProcessPromises.values());
    }
  }
...
---`;
  ...
  async stopWatching(watchPath = null) {
    if (watchPath && this.watchers.has(watchPath)) {
      ...
      await this.flushPendingChanges(watchPath);
